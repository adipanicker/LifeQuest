  ...
  return (
    <div className="relative">
      <div className="flex items-center gap-3 py-3 border-b border-gray-100 last:border-none">
        ...
        {/* Goal tag or attach button — only show if not completed */}
        {!todo.is_completed && (
          <>
            {linkedGoal ? (
              <button
                onClick={() => setShowGoalPicker(!showGoalPicker)}
                className="flex items-center gap-1 text-xs bg-primary-light text-primary-dark px-2 py-0.5 rounded-full hover:bg-primary hover:text-white transition-colors"
              >
                🎯{" "}
              ...
              <button
                onClick={() => setShowGoalPicker(!showGoalPicker)}
                className="text-xs text-gray-400 border border-dashed border-gray-300 px-2 py-0.5 rounded-full hover:border-primary hover:text-primary transition-colors"
              >
                + Goal
              </button>
            )}
      ...
      {showGoalPicker && (
        <>
          ...
          <div className="absolute right-6 top-10 z-20 bg-white border border-gray-200 rounded-xl shadow-lg p-2 min-w-48">
            ...
            {todo.goal_id && (
              <>
                ...
                <button
                  ...
                  className="w-full text-left text-sm px-2 py-1.5 rounded-lg text-red-400 hover:bg-red-50 transition-colors"
                >
                  Remove link
                </button>
              </>
